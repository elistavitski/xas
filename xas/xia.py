...
In [1]: all_counts=np.zeros(12) 
   ...
   ...:             counts.append(np.sum(c.iloc[mca-1][0].data[p_lo:p_hi+1])) 
   ...:     all_counts[roi] = sum(counts)  
...
class xiaparser:
            ...
            for ds_index, ds in enumerate(self.data):
                for board_number, curr_ds in enumerate(ds):
                    ...
                    for i in range(number_pixels):
                        self.next_pos = self.read_pixel_block(curr_ds, board_number, i, self.next_pos, plotdata,
                                                              pixelnumber, silent)
        ...
        if all_in_one:
            ...
            with open(dest_filepath + tmpfilename + '-allchans.txt', 'wb') as f:
                ...
                for row in output_data:
                    ...
                    i += 1
                    np.savetxt(f, np.array(row), fmt='%i', delimiter=' ',
                               footer='============================================================')
    ...
    def read_pixel_block(self, dataset, board_number, pixel, start_pos, plot_data, pixel_to_parse, silent):
        ...
        channel_0_stats_realtime, channel_0_stats_livetime, channel_0_stats_triggers, channel_0_stats_output_evs = read_statistics(
            dataset, pos + 32)
        channel_1_stats_realtime, channel_1_stats_livetime, channel_1_stats_triggers, channel_1_stats_output_evs = read_statistics(
            dataset, pos + 40)
        channel_2_stats_realtime, channel_2_stats_livetime, channel_2_stats_triggers, channel_2_stats_output_evs = read_statistics(
            dataset, pos + 48)
        channel_3_stats_realtime, channel_3_stats_livetime, channel_3_stats_triggers, channel_3_stats_output_evs = read_statistics(
            dataset, pos + 56)
        ...
        if ((pixel_number == number or pixel_to_parse == None) and (not silent)):
            ...
            if plot_data:  # and len(channel_0_spectrum):
                ...
                ax = fig.gca()
                ax.plot(channel_0_spectrum,
                        label="Channel {} / RT: {} / LT: {}".format((4 * board_number) + 0, channel_0_stats_realtime,
                                                                    channel_0_stats_livetime))
                ax.plot(channel_1_spectrum,
                        label="Channel {} / RT: {} / LT: {}".format((4 * board_number) + 1, channel_1_stats_realtime,
                                                                    channel_1_stats_livetime))
                ax.plot(channel_2_spectrum,
                        label="Channel {} / RT: {} / LT: {}".format((4 * board_number) + 2, channel_2_stats_realtime,
                                                                    channel_2_stats_livetime))
                ax.plot(channel_3_spectrum,
                        label="Channel {} / RT: {} / LT: {}".format((4 * board_number) + 3, channel_3_stats_realtime,
    ...
    def read_header(self, dataset, print_headers, silent):
        ...
        buffer_errors = dataset[
            24]  # Buffer errors: Buffer overrun 0: No error / >0: Number of extra pixels combined with last pixel in buffer
    ...
    def parse_roi(self, pixels, channel_number, rois=[[0, 20]], energy_range=20):  # min_energy = 0, max_energy = 20):
        ...
        for roi in rois:
            ...
            roi_integrations.append(
                list([v.values[channel_number - 1] @ condition.astype(int) for k, v in self.df.items()])[:pix_max])

        return np.array(roi_integrations)

    def plot_roi(self, filename, filepath, pixels, channel_number, min_energy=0, max_energy=20, ax=plt,
                 energy_array=np.array([]), background=[]):
    ...
    def parse_step_scan(set_name, roi_start, roi_end, path='/GPFS/xf08id/xia_files/'):
        ...
        det_channels.append(
            np.array(det_channels[2]) + np.array(det_channels[3]) + np.array(det_channels[4]) + np.array(
                det_channels[5]))
